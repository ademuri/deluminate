...
export function Site(url) {
  ...
  this.page_hierarchy = url_object.pathname.split('/').filter(function (x) {
    return Boolean(x);
  });
...
export function Settings(defaultFilter, defaultMods) {
  ...
  this.match = function (site) {
    ...
    if (site_domain instanceof Hierarchy) {
      return Site.build(domain_path, site_domain.getMatch(site.page_hierarchy));
    }
